...
def filter_relation(relation,start,end,temp):
...
def dump_file(relation,rows):
	fp_out=open(relation+'_filter.txt','w')
...
if __name__=='__main__':
	relation=sys.argv[1]
	#rows=load_file(relation)
	#dump_file(relation,rows)
	rows=load_stc_file(relation)
	dump_file(relation,rows)
